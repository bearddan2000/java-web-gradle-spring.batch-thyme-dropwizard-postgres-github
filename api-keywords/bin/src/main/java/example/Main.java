...
public class Main extends Application<DbConfiguration> {
    ...
    public void run(DbConfiguration configuration,
        ...
        // nothing to do yet
        KeywordsDao dao = new KeywordsDao(hibernate.getSessionFactory());
        final KeywordsResource resource = new KeywordsResource(dao);
        e.jersey().register(resource);
    }

    private HibernateBundle<DbConfiguration> hibernate = new HibernateBundle<DbConfiguration>(Keywords.class) {
        @Override
